...
async def send_invite_email(
    ...
    """Send the invite via Resend HTTPS API."""
    import httpx
    ...
    payload = {
        "from": "Wanderly <onboarding@resend.dev>",
        "to": [to_email],
        "subject": f"\u2708\ufe0f Pack your bags \u2014 {trip_name}",
        "html": html_body,
        "text": text_body,
    }
    ...
    result = resp.json()
    log.info("Email sent via Resend: id=%s to=%s", result.get("id"), to_email)
